...
#
#================================================================
# download_oasis_scans_http.sh
#================================================================
#
# Usage: ./download_oasis_scans_httpie.sh <input_file.csv> <directory_name> <xnat_central_username> <scan_type>
# 
# Download scans of a specified type from OASIS3 on XNAT Central and organize the files - uses "httpie" instead of "curl"
#
# You can learn how to install and use httpie at https://httpie.org
#
# Required inputs:
...
if [ ${#@} == 0 ]; then
    ...
    sed 1d $INFILE | while IFS=, read -r EXPERIMENT_ID; do
        ...
        download_url=https://central.xnat.org/data/archive/projects/OASIS3/subjects/${SUBJECT_ID}/experiments/${EXPERIMENT_ID}/scans/${SCANTYPE}/files

        download ${OUTPUT_PREFIX} "$download_url" tar.gz
